 ...
 */
class risk extends Admin_Controller {
    ...
    function __construct() {
        ...
        if ($this->input->post('cancel-button'))
            redirect('master/risk/index');
    }

    function index() {
		$this->data['rows'] = $this->risk_model->get_list(site_url('master/risk/index'));
		$this->template->build('risk-list');
    }
	...
	function view($id) {
        ...
        $this->data['risk_category'] = $this->risk_model->get_by_risk_category($riskCategory->risk_category_id);
		$this->data['status'] = $this->risk_model->get_by_status($user->status);
		$this->data['unit'] = $this->risk_model->get_by_unit($user->unit_id);
		
		$this->data['row'] = $this->risk_model->get_by_id((int) $id);
        $this->template->build('risk-view');
    }
    ...
    function _updatedata($id = 0) {
		...
		$this->data['id'] = '';
        $this->data['code'] = $this->risk_model->create_code();
        ...
        if ($id > 0) {
            $row = $this->risk_model->get_by_id((int) $id);
        ...
        if ($this->form_validation->run()) {
            if ($id > 0) {
                $this->risk_model->update($id, $this->form_validation->get_values());
				$this->template->set_flashdata('info', 'Data has been updated');
            } else {
                $this->risk_model->insert($this->form_validation->get_values());
            ...
            redirect('master/risk');
        }

        $this->data['form'] = $this->form_validation;
        $this->template->build('risk-form');
    }

    function delete($id) {
		$risk = $this->risk_model->get_by_id($id);
		if ($risk)
			$this->risk_model->delete($id);
		/* if update status */
        /* $this->risk_model->update($id, array('status' => 0)); */
        redirect('master/risk');
    }
	...
	function do_upload_photo($id) {
        ...
        if (!$this->upload->do_upload()) {
            ...
            if($id > 0){
                $this->risk_model->update($id, array('photo' => $img));
            }  else {
                print_r($error);
            }
        }
