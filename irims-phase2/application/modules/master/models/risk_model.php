...
class risk_model extends MY_Model {
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/RISK/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
    ...
    function get_by_risk_category($id) {
        $this->db->select($this->risk_category_table . '.*, ' . $this->risk_category_table . '.name AS risk_category_name')
    ...
    function get_by_risk_category_report($id) {
        $query = $this->db->get_where($this->risk_category_table, array($this->risk_category_table . '.' . $this->id_field => $id));
        if ($query->num_rows() > 0)
            return $query->row();
        else
            return FALSE;
    ...
    function get_by_unit_report($id) {
        $query = $this->db->get_where($this->unit_table, array($this->unit_table . '.' . $this->id_field => $id));
        if ($query->num_rows() > 0)
            return $query->row();
        else
            return FALSE;
    ...
    function get_by_risk_report($id) {
        $query = $this->db->get_where($this->table, array($this->table . '.' . $this->id_field => $id));
        if ($query->num_rows() > 0)
            return $query->row();
        else
