...
/**
 * Risk Impact Indicator Model
 * 
 * @package App
 * @category Model
 * @author Wildan Sawaludin
 */
class risk_impact_indicator_model extends MY_Model {
	
	private $ci;
	protected $table = 'mst_risk_impact_indicators';
    protected $risk_impact_category_table = 'mst_risk_impact_categories';
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/RII/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
    ...
    function get_by_group_risk_impact_category() {
        $data   = array();
        $data   = $this->db->order_by('id', 'asc')->get($this->risk_impact_category_table)->result_array();
        foreach( $data as $key=>$each ){
           $data[$key][$this->table]   = $this->db->order_by('id', 'asc')->where('mst_risk_impact_category_id', $each['id'])->get($this->table)->result_array();  
        }
        return $data;
	}
	...
	function insert($data) {
        ...
        $data['mst_risk_impact_id_A'] = RISK_IMPACT_VALUE_A;
        $data['mst_risk_impact_id_B'] = RISK_IMPACT_VALUE_B;
        $data['mst_risk_impact_id_C'] = RISK_IMPACT_VALUE_C;
        $data['mst_risk_impact_id_D'] = RISK_IMPACT_VALUE_D;
        $data['mst_risk_impact_id_E'] = RISK_IMPACT_VALUE_E;
    ...
    function get_by_risk_impact_category($id) {
        $this->db->select($this->risk_impact_category_table . '.*, ' . $this->risk_impact_category_table . '.name AS risk_name')
                ->join($this->risk_impact_category_table, $this->risk_impact_category_table . '.id = ' . $this->table . '.mst_risk_impact_category_id', 'left');
        return parent::get_by_id($id);
    }
