 ...
 */
class risk_probability_model extends MY_Model {
	
	private $ci;
	protected $table = 'mst_risk_probabilities';
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/PRO/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
	
	function get_list($base_url = '', $offset = 0, $limit = 0) {
        return $this->db->order_by('frequency', 'asc')->get_where($this->table, array(
				$this->table . '.status' => 1, 
    ...
    function get_by_frequency($frequency) {
        $query = $this->db->get_where($this->table, array(
			$this->table . '.frequency'=> $frequency, 
			$this->table . '.status' => 1, 
			/*$this->table . '.unit_id' => $this->session->userdata('unit_id')*/));
        if ($query->num_rows() > 0)
            return $query->row();
    ...
    function get_all_root() {
        $query = $this->db->select('id, code, frequency')
    ...
    function get_all_by_parent($parentCode) {
        $query = $this->db->select('id, frequency')
                ->order_by('frequency', 'ASC')
                ->get_where($this->table, array(
	...
	function get_search($param = NULL) {
		$query = $this->db->where(array(
			$this->table . '.unit_id' => $this->session->userdata('unit_id')))
			->like('LOWER('. $this->table.'.frequency)', strtolower($param), 'both', FALSE)
                                  ->or_like('LOWER('. $this->table.'.frequency)', strtolower($param), 'both', FALSE)
                                  ->get($this->table);
		if ($query->num_rows() > 0)
			return $query->result();
		else
	...
	function drop_options() {
        $query = $this->db->select('id, rating_value')
                ->order_by('id', 'ASC')
                ->get_where($this->table, array(
				/*$this->table . '.unit_id' => $this->session->userdata('unit_id'),*/ 
				$this->table . '.status' => 1));
        ...
        foreach ($result as $item) {
            $options[$item->id] = $item->rating_value;
        }
        return $options;
    }
	...
	function drop_options_root() {
        ...
        $options[''] = '';
        foreach ($result as $item) {
            $options[$item->id] = $item->frequency;
        }
        return $options;
    }
	...
	function autocomplete() {
        $query = $this->db->select('id, frequency')
                ...
                ->like('upper(frequency)', strtoupper($_GET['term']))
                ->order_by('frequency', 'ASC')
                ->get($this->table);
        ...
        foreach ($result as $row) {
            $arrData[] = array('id' => $row->id, 'value' => $row->frequency);
        }
        return $arrData;
    }
	...
	function insert($data) {
        ...
        $data['frequency'] = str_replace('&amp;', '&', $_POST['frequency']);
        $data['quantitative_criteria'] = str_replace('&amp;', '&', $_POST['quantitative_criteria']);
        $data['qualitative_criteria'] = str_replace('&amp;', '&', $_POST['qualitative_criteria']);
