...
/**
 * Exco Effectiveness Question Model
 * 
 * @package App
 * @category Model
 * @author Wildan Sawaludin
 */
class exco_effectiveness_question_model extends MY_Model {
	
	private $ci;
	protected $table = 'mst_exco_effectiveness_questions';
    protected $exco_effectiveness_answers_table = 'mst_exco_effectiveness_answers';
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/EEQ/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
    ...
    function getQuestionAndAnswerChoice() {
        $this->db->select('x.id as `question_id`, x.name as `question`, y.aspect_value as `option_id`, y.name as `option`');
        $this->db->from('mst_exco_effectiveness_questions as `x`');
        $this->db->join('mst_exco_effectiveness_answers as `y`', 'y.exco_effectiveness_question_id = x.id', 'left');
        $this->db->where('x.status', '1');
        $this->db->order_by('x.id, y.id', 'asc');
        $query = $this->db->get();
        ...
        foreach ($result as $row)
        {
            ...
            if (!array_key_exists($question_id, $shaped_result))
            {
                $shaped_result[$question_id] = array('question_id' => $question_id, 'question' => $row['question'], 'answers' => array());
            }

            if ($row['option'] != null)
            {
                // Push B item onto sub array
                $shaped_result[$question_id]['answers'][] = array('option_id' => $row['option_id'], 'option' => $row['option']);
            }
        }
