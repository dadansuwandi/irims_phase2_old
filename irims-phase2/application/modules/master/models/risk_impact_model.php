 ...
 */
class risk_impact_model extends MY_Model {
	
	private $ci;
	protected $table = 'mst_risk_impacts';
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/IMP/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
	...
	function get_search($param = NULL) {
		$query = $this->db->like('LOWER('. $this->table.'.name)', strtolower($param), 'both', FALSE)
	...
	function drop_options() {
        $query = $this->db->select('id, alphabet')
                ->order_by('alphabet', 'ASC')
                ->get_where($this->table, array(
				/*$this->table . '.unit_id' => $this->session->userdata('unit_id'),*/ 
				$this->table . '.status' => 1));
        $result = $query->result();
