...
/**
 * Exco Effectiveness Value Category Model
 * 
 * @package App
 * @category Model
 * @author Wildan Sawaludin
 */
class exco_effectiveness_value_category_model extends MY_Model {
	
	private $ci;
	protected $table = 'mst_exco_effectiveness_value_categories';
	...
	function create_code() {
        ...
        $nextNo = substr('00'.$nextNo, -3) . '/EEVC/' . date('m') . '/' . date('Y');
        return $nextNo;
    }
    ...
    function getExcoEffectivenessValueCategoryByGroupValue($groupValue) {
        $query = $this->db->get_where($this->table, array(
			$this->table . '.group_value'=> $groupValue, 
			$this->table . '.status' => 1));
        if ($query->num_rows() > 0)
            return $query->row();
        else
    ...
    function getExcoEffectivenessValueCategoryByGroupLevel($groupValue) {
        $query = $this->db->get_where($this->table, array(
			$this->table . '.group_value'=> $groupValue, 
			$this->table . '.status' => 1));
        if ($query->num_rows() > 0)
    ...
    function getExcoEffectivenessValueCategoryByGroupName($groupValue) {
        $query = $this->db->get_where($this->table, array(
			$this->table . '.group_value'=> $groupValue, 
			$this->table . '.status' => 1));
        if ($query->num_rows() > 0)
