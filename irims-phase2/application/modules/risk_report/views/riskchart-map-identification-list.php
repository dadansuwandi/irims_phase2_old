...
.risk-box.dahsyat-risk{
	...
	background-image: url("../../../assets/img/riskchart_map/dahsyat.png");
...
.risk-box.besar-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/besar.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.menengah-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/menengah.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.rendah-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/rendah.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.tidak_significant-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/tidak_significant.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.keterangan-risk{
	...
	background-image: url("../../../assets/img/riskchart_map/keterangan.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
	height: 100px;
...
.risk-box.sangat_kecil-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/sangat_kecil.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.kecil-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/kecil.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.sedang-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/sedang.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.besar2-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/besar2.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
.risk-box.hampir_pasti-risk{
	background:rgba(255,255,255,.8);
	border:1px solid rgba(168,168,168,1);
	background-image: url("../../../assets/img/riskchart_map/hampir_pasti.png");
	background-repeat: no-repeat;
	background-size: 100%;
	background-position: center;
}
	...
	<div class="portlet-body form">
		...
		<form action="" method="POST" class="horizontal-form">
			...
			<div class="form-actions">
				...
				<a href="<?php echo site_url('risk_report/riskchart_map/index_identification'); ?>" class="btn default">Reset</a> 
				<!-- <a href="<?php //echo site_url('risk_report/riskchart_map/index_identification_pdf?tahun='.$_POST['tahun']); ?>" class="btn red">Export to PDF<i class="fa fa-file-pdf-o"></i></a>  -->
	...
	<div class="col-md-6">
		...
		<div class="portlet box yellow">
			...
			<div class="portlet-body">
				...
				<!-- END MAP MATRIX-->
				<!-- BEGIN RISK CHART-->
				<!-- id div wajib diberi nama risk-chart-map-d3 -->
				<div id="risk-chart-map-d3" style="height: 100%; width:100%;"></div><!-- widht dan height bisa diatur sesuai kebutuhan, bisa memakai % -->
				<?php
				$i = 1;
				$tmp = array();
				foreach ($rows as $row):
				...
				?>
				<script>
					drawRiskChartMap(<?php echo json_encode($output)?>);
				</script>
				<div id="risk-chart-map-d3-temp" style="height: 100%; width:100%;"><canvas id="printCanvas"></canvas></div>
				<!-- END RISK CHART-->
				...
				<div class="portlet-body flip-scroll">
					<table class="table table-bordered table-striped table-condensed flip-content">
						...
						<tr>
							<td width="20%" bgcolor="#72DAD7">&nbsp;</td>
							<td>Risiko Menengah (<i>Medium)</i></td>
							<td>&nbsp;</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>Risiko Extrem (<i>Extreme)</i></td>
							<td>&nbsp;</td>
						</tr>
						</tbody>
					</table>
...
<script>
	...
	function showEvent(risk_item_id)
	{
		...
		$.ajax({
	        url: "<?php echo site_url('risk_report/riskchart_map/get_event_detail_mitigation');?>",
	        type: "POST",
	...
	$("#risk-chart-map-d3-temp").hide();
	$("#pdfDownloader").click(function(){
		//show div before canvas will be generated
		$("#risk-chart-map-d3-temp").show();

		var canvas = document.getElementById("printCanvas");
		
		//Get svg markup as string
		var svg = document.getElementById('risk-chart-map-d3').innerHTML;
        ...
        html2canvas(document.getElementById("printDiv"), {
            onrendered: function(canvas) {
                ...
                doc.save($.now()+'_riskchart_map_identification_list.pdf');
            }
        });
