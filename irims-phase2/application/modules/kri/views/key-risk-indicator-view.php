			...
			<div class="portlet-body">
				...
				<table class="table table-striped table-bordered table-hover" id="sample_6">
					...
					<tbody>
						<?php
						$i = 0;
						$key_risk_indicator_threshold_value = $this->key_risk_indicator_threshold_value_model->get_data($row->KEY_RISK_INDICATOR_ID);
						//foreach ($rows as $row):
							$i++;
							?>
							<tr>
                ...
                <div class="row">
                    <div class="col-md-12">
						...
						<?php if($this->session->userdata('role_id')==GROUP_RISK_ADMIN) { ?>
                        	<a href="<?php echo site_url('kri/key_risk_indicator/index'); ?>" class="btn yellow"><i class="fa fa-chevron-left"></i> KEMBALI</a> 
						<?php } else if($this->session->userdata('role_id')==GROUP_RISK_OWNER) { ?>
							<a href="<?php echo site_url('kri/key_risk_indicator/index_list'); ?>" class="btn yellow"><i class="fa fa-chevron-left"></i> KEMBALI</a> 
						<?php } ?>
						...
						?>
						<a href="<?php echo site_url('kri/key_risk_indicator_threshold_value/measure_value/'.$row->KEY_RISK_INDICATOR_ID); ?>" class="btn green">INPUT MEASURE VALUE<i class="fa fa-pencil"></i></a>
						<?php	
							//} else {
						?>
						<!-- <a href="<?php //echo site_url('kri/key_risk_indicator_threshold_value/measure_value/'.$row->KEY_RISK_INDICATOR_ID); ?>" class="btn blue">EDIT<i class="fa fa-pencil"></i></a> -->
						<?php
							//}
						?>
                    </div>
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="portlet box blue-hoki">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-list"></i>Treshold Value
				</div>
			</div>
			<div class="portlet-body">
				<table class="table table-striped table-bordered table-hover" id="sample_5">
					<thead>
						<tr>
							...
							<th class="hidden-xs">Threshold Value</th>
							<th class="hidden-xs">Input Date</th>
					...
					<tbody>
						...
						foreach ($rows as $row):
							...
							<tr>
								...
								<td>
									<?php
									$kri = $this->key_risk_indicator_model->get_by_id($row->key_risk_indicator_id);
									
									$Indicator = $this->indicator_model->get_by_id($kri->INDICATOR_ID);
									if ($Indicator)
										echo $Indicator->name;
									else
										echo '-';
								...
								<?php if($this->session->userdata('role_id')==GROUP_ADMINISTRATOR || $this->session->userdata('role_id')==GROUP_RISK_ADMIN):?>
								<td style="width: 14%; align=center;">
									<a href="<?php echo site_url('kri/key_risk_indicator_threshold_value/measure_value/'.$row->key_risk_indicator_id.'?threshold_value_id='.$row->id); ?>" title="Edit" class="btn btn-xs blue"><i class="fa fa-edit"></i></a>
									<a href="<?php echo site_url('kri/key_risk_indicator_threshold_value/delete/'.$row->id); ?>" title="Delete" class="btn btn-xs red" data-button="delete"> <i class="fa fa-times"></i></a>
								</td>
								<?php endif;?>
							</tr>
						<?php endforeach; ?>
