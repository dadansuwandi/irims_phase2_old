<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    /**
     * Risk Evaluation Worksheet Controller.
     * 
     * @package App
     * @category Controller
     * @author Wildan Sawaludin
     */
    class risk_evaluation_worksheet extends Admin_Controller {
        ...
        function __construct() {
            ...
            if ($this->input->post('cancel-button'))
                redirect('risk_worksheet/risk_evaluation_worksheet/index');
        }

        function index() {
    		$this->data['rows'] = $this->risk_identification_model->get_list_admin_bumn(array(4,5,6));
    		$this->template->build('risk-evaluation-worksheet-list');
        }
        ...
        function event() {
            ...
            $this->template->build('risk-evaluation-worksheet-event-list');
        }

         function event_gm() {
            $this->data['rows'] = $this->risk_identification_model->get_list_gm(array(4,5,6));
            $this->template->build('risk-evaluation-worksheet-event-gm-list');
        }
    	...
    	function view($id) {
            ...
            $this->template->build('risk-evaluation-worksheet-view');
        }
        ...
        function _updatedata($id = 0) {
            ...
            if ($id > 0) {
               ...
               /* $this->data['RENCANA_PENGENDALIAN']             = $row->RENCANA_PENGENDALIAN;
                ...
                $this->data['QUANTIFICATION_ANALISYS']           = $row->QUANTIFICATION_ANALISYS;

                $this->data['NOTES_CURRENT_RISK_K']              = $row->NOTES_CURRENT_RISK_K;
                $this->data['NOTES_CURRENT_RISK_D']              = $row->NOTES_CURRENT_RISK_D;
                $this->data['NOTES_TARGET_RESIDUAL_RISK_K']      = $row->NOTES_TARGET_RESIDUAL_RISK_K;
                $this->data['NOTES_TARGET_RESIDUAL_RISK_D']      = $row->NOTES_TARGET_RESIDUAL_RISK_D;
            ...
            if ($this->form_validation->run()) {
                ...
                redirect('risk_worksheet/risk_evaluation_worksheet');
            }
            ...
            $this->template
                ...
                ->build('risk-evaluation-worksheet-form');
        }
        ...
        function saveWizard() {
            ...
            if(isset($_POST)){
                ...
                $dataPostIdentification['MITIGASI_RISK_K_ID']                = $_POST['MITIGASI_RISK_K_ID'];
                $dataPostIdentification['MITIGASI_RISK_D_ID']                = $_POST['MITIGASI_RISK_D_ID'];
                $dataPostIdentification['QUANTIFICATION_ANALISYS']           = $_POST['QUANTIFICATION_ANALISYS'];

                $dataPostIdentification['NOTES_CURRENT_RISK_K']              = $_POST['NOTES_CURRENT_RISK_K'];
                $dataPostIdentification['NOTES_CURRENT_RISK_D']              = $_POST['NOTES_CURRENT_RISK_D'];
                $dataPostIdentification['NOTES_TARGET_RESIDUAL_RISK_K']      = $_POST['NOTES_TARGET_RESIDUAL_RISK_K'];
                $dataPostIdentification['NOTES_TARGET_RESIDUAL_RISK_D']      = $_POST['NOTES_TARGET_RESIDUAL_RISK_D'];

                $dataPostIdentification['TIPE_KERTAS_KERJA']                 = TIPE_KERTAS_KERJA_BUMN;
                ...
                if($result){
                    ...
                    if(isset($_POST['RENCANA_PENGENDALIAN']) && count($_POST['RENCANA_PENGENDALIAN']) > 1 && $_POST['RENCANA_PENGENDALIAN'][1] != ""){
                        ...
                        $riskMitigationData['EXECUTION_TIME']        = $_POST['EXECUTION_TIME'];
                        $riskMitigationData['YEAR']                  = date("Y");
                        $riskMitigationData['MONTH']                 = date("m");
                        $riskMitigationData['DAY']                   = date("d");
