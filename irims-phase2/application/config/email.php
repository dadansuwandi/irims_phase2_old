...
$config['useragent']        = getenv('USERAGENT');              // Mail engine switcher: 'CodeIgniter' or 'PHPMailer'
$config['protocol']         = getenv('PROTOCOL');                   // 'mail', 'sendmail', or 'smtp'
$config['mailpath']         = '/usr/sbin/sendmail';
$config['smtp_host']        = getenv('SMTP_HOST');
$config['smtp_user']        = getenv('SMTP_USER');
$config['smtp_pass']        = getenv('SMTP_PASS');
$config['smtp_port']        = getenv('SMTP_PORT');
$config['smtp_timeout']     = getenv('SMTP_TIMEOUT');                       // (in seconds)
$config['smtp_crypto']      = getenv('SMTP_CRYPTO');                       // '' or 'tls' or 'ssl'
$config['smtp_debug']       = getenv('SMTP_DEBUG');                        // PHPMailer's SMTP debug info level: 0 = off, 1 = commands, 2 = commands and data, 3 = as 2 plus connection status, 4 = low level data output.
...
$config['wrapchars']        = 76;
$config['mailtype']         = getenv('MAILTYPE');                   // 'text' or 'html'
$config['charset']          = getenv('CHARSET');                     // 'UTF-8', 'ISO-8859-15', ...; NULL (preferable) means config_item('charset'), i.e. the character set of the site.
$config['validate']         = true;
$config['priority']         = 3;                        // 1, 2, 3, 4, 5; on PHPMailer useragent NULL is a possible option, it means that X-priority header is not set at all, see https://github.com/PHPMailer/PHPMailer/issues/449
$config['crlf']             = getenv('CRLF');                     // "\r\n" or "\n" or "\r"
$config['newline']          = getenv('NEWLINE');                     // "\r\n" or "\n" or "\r"
